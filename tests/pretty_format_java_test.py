...
from language_formatters_pre_commit_hooks.pretty_format_java import (
    _download_google_java_formatter_jar,
)
...
def test__download_google_java_formatter_jar(
...
@pytest.mark.skipif(
    condition=get_jdk_version() < Version("16"),
    reason="Skipping test because it requires Java JDK 16+",
)
def test_pretty_format_java_up_to_1_9_is_not_allowed_on_jdk_16_and_above(
    undecorate_method,
):
    with pytest.raises(
        ToolNotInstalled,
        match="JRE: version < 16.0 is required to run this pre-commit hook.",
    ):
        undecorate_method(
            ["--google-java-formatter-version=1.9", "pretty-formatted.java"]
        )


@pytest.mark.skipif(
    condition=get_jdk_version() >= Version("16"),
    reason="Skipping test because it requires Java JDK before 16",
)
...
@patch(
    "language_formatters_pre_commit_hooks.pretty_format_java.run_command", autospec=True
)
def test_pretty_format_java_jar(
    mock_run_command, undecorate_method, cli_arg, expected_retval
):
